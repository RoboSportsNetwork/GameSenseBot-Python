...
@example('!record 148 2015', 'Overall, team 148 had an average qual score of 116.10 and an average playoff score of 130.13 in 2015.')
def record(bot, trigger):
    ...
    if year != 0:
        ...
        if not matches:
            ...
            if year == '2015':
                averages = communicator.getTeamScoreAvgInMatches('frc'+teamNumber, matches)
                bot.reply(
                    str(teamNumber) + 'had an average qual score of ' + '{0:.2f}'.format(averages.qualAvg) + 'and an average playoff score of ' + '{0:.2f}'.format(averages.playoffAvg) + ' in ' + str(year) + '.')
            else:
                bot.reply(
                    str(teamNumber) + ' was ' +
        ...
        if not matches:
            ...
            if eventCode.startswith('2015'):
                averages = communicator.getTeamScoreAvgInMatches('frc'+teamNumber, matches)
                bot.reply(
                    str(teamNumber) + 'had an average qual score of ' + '{0:.2f}'.format(averages.qualAvg) + 'and an average playoff score of ' + '{0:.2f}'.format(averages.playoffAvg) + ' at the ' + str(event.year) + ' ' + event.toString() + '.')
            else:
                bot.reply(
                    str(teamNumber) + ' was ' +
                    communicator.getTeamRecordInMatches('frc' +
